...
interface RecentActivity {
  id: string;
  type: "exam_started" | "exam_completed" | "exam_registered" | "violation" | "login";
  user_id: string;
  details: string;
  timestamp: string;
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch security activity logs for recent activities
      ...
      const totalStudents = new Set(sessions?.map(s => s.user_id)).size || 0;
      const activeExams = sessions?.filter(s => s.submission_status === "in_progress").length || 0;
      const completedToday = sessions?.filter(s => 
        s.submission_status === "submitted" && 
        new Date(s.end_time || "") >= todayStart
      ).length || 0;
      
      const completedExams = sessions?.filter(s => s.submission_status === "submitted" && s.score !== null) || [];
      const avgScore = completedExams.length > 0 
        ? Math.round(completedExams.reduce((sum, exam) => sum + (exam.score || 0), 0) / completedExams.length)
        : 0;

      // Get recent exams (last 5)
      const recentExamsData = (sessions || [])
      ...
      const transformedActivities = (activities || []).map(activity => ({
        id: activity.id,
        type: (activity.activity_type === "tab_switch" ? "violation" :
              activity.activity_type === "multiple_faces" ? "violation" :
              activity.activity_type === "face_detected" ? "exam_started" :
              "exam_completed") as RecentActivity["type"],
        user_id: activity.exam_session_id || "unknown",
        details: activity.details || activity.activity_type,
      ...
      setStats({
        ...
        completedToday,
        avgScore,
        studentGrowth: 12, // Mock growth percentages
        examGrowth: 8,
      });
      ...
      // Set realtime stats
      setRealtimeStats({
        activeUsers: activeExams,
        currentSessions: activeExams,
        violationsToday: activities?.filter(a => 
          a.activity_type === "tab_switch" || a.activity_type === "multiple_faces"
        ).length || 0,
      });
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{realtimeStats.currentSessions}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{realtimeStats.violationsToday}</p>
            ...
            <CardContent>
              <div className="space-y-4">
                {recentExams.map((exam, index) => (
                  <div key={exam.id || index} className="flex items-center justify-between p-4 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
