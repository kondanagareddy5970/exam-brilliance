...
const TakeExam = () => {
  ...
  useEffect(() => {
    if (!examStarted || !webcamEnabled) return;
    ...
    navigator.mediaDevices.getUserMedia({ video: true, audio: false }).then((stream) => {
      ...
      // Join a room (could use examId + user id)
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          socketRef.current.emit('signal', { room: `exam-${examId}`, data: { candidate: event.candidate } });
        }
      };
      // Offer to admin
      pc.createOffer().then(offer => {
        pc.setLocalDescription(offer);
        socketRef.current.emit('signal', { room: `exam-${examId}`, data: { sdp: offer } });
      });
      // Listen for admin answer/candidates
  ...
  }, [examStarted, webcamEnabled, examId]);
